...
/// </summary>
internal sealed class EventChangeHandler
{
    /// <summary>
    ...
    public EventStatusChangeDecision ApplyChanges(domain.Event existing, domain.Event updated)
    {
        ...
        bool shouldSendPlanned =
            originalStatus != domain.EventStatus.Planned &&
            updated.Status == domain.EventStatus.Planned &&
            !originalNotificationSent &&
            canContact;

        bool shouldSendInvoice =
            originalStatus != domain.EventStatus.SendInvoice &&
            updated.Status == domain.EventStatus.SendInvoice &&
            canContact;
