...
internal class ShiftService : IShiftService
{
    ...
    public async Task AddStaffToShiftAsync(Shift shift, Staff staff)
    {
        ...
        if (!staff.IsActive)
        {
            throw new DomainException($"Staff member {staff.ReferenceId} is not active.");
        }
        ...
        if (!isAvailable)
        {
            throw new DomainException($"Staff member {staff.ReferenceId} is not available for this shift.");
        }
    ...
    public async Task RemoveStaffFromShiftAsync(Shift shift, Staff staff)
    {
        ...
        if (assignment == null)
        {
            throw new DomainException($"Staff member {staff.ReferenceId} is not assigned to this shift.");
        }
